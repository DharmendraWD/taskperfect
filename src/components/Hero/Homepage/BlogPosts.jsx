  ...
  {parse(first3BlogsData?.data?.items?.[0].blogDesc)}
        ...
        <div className="grid grid-cols-1 gap-8">
          {first3BlogsData?.data?.items.slice(1, 3).map((blog, index) => (
            <div key={index} className="flex allCards flex-col h-[50%] md:flex-row  bg-transparent rounded-lg overflow-hidden m-auto">
              <Link to={"/blog/"+blog.id} state={blog} className='h-[100%] flex items-center'>
               <div className='flex flex-col md:flex-row h-[50%] items-center gap-4 bg-transparent rounded-lg overflow-hidden'>
