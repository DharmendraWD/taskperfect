...
import { useDispatch, useSelector } from 'react-redux';
import { totalPromoshare } from '../../../redux/slices/promoShare/PromoshareSlice';
import { promoshareStatus } from '../../../redux/slices/promoShare/PromoshareSlice';
...
function PromoShare() {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 justify-self-center md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
        {
          data.map((promoshare, index) => (
            <PromoShareCard
              title={promoshare.title}
              desc={promoshare.description}
              img={promoshare.thumbnail}
              key={index}
            />
          ))
        }
